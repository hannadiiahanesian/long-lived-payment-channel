...
contract(
  ...
  (accounts) => {
    ...
    it("recepient is able to withdraw", async () => {
      const withdrawalSum = web3.utils.toWei("1", "ether");
      const contractStartBalance = await web3.eth.getBalance(contractAddress);
      const recipientStartBalance = await web3.eth.getBalance(recipient);

      const msgHash = web3.utils.soliditySha3(
        { t: "address", v: contractAddress },
        { t: "uint256", v: withdrawalSum }
      );

      const signed = await web3.eth.accounts.sign(msgHash, senderSkey);

      const withdrawalTX = await longLivedPaymentChannelTx.methods
        .withdraw(withdrawalSum, signed.signature)
        .send({ from: recipient });

      const recipientAfterBalance = await web3.eth.getBalance(recipient);
      const contractAfterBalance = await web3.eth.getBalance(contractAddress);

      const tx = await web3.eth.getTransaction(withdrawalTX.transactionHash);

      const Fee = web3.utils
        .toBN(tx.gasPrice)
        .mul(web3.utils.toBN(withdrawalTX.gasUsed));

      const recipientSumBalance = web3.utils
        .toBN(recipientStartBalance)
        .add(web3.utils.toBN(withdrawalSum))
        .sub(web3.utils.toBN(Fee));

      const contractSumBalance = web3.utils
        .toBN(contractStartBalance)
        .sub(web3.utils.toBN(withdrawalSum));

      assert.equal(recipientSumBalance, recipientAfterBalance, "Invalid recipient balance");
      assert.equal(contractSumBalance, contractAfterBalance, "Invalid contract balance");
    });
  }
);
